...
export const passwordresetanaly = async (req, res) => {
  try {
    ...
    // Create a map to store user history
    const userHistoryMap = new Map();
    
    // First, get all user IDs from the current result set
    const userIds = events.map(event => event.userId?._id).filter(id => id);
    
    // For each unique user ID, fetch their previous password reset history
    // (excluding the ones already in our current result set)
    if (userIds.length > 0) {
      // Get the oldest timestamp from current results to use as upper bound
      const oldestEventTimestamp = events.length > 0 ? 
        new Date(Math.min(...events.map(e => new Date(e.timestamp).getTime()))) : 
        new Date();
        
      // Find previous events for these users
      const previousEvents = await PasswordResetEvent.find({
        userId: { $in: userIds },
        timestamp: { $lt: oldestEventTimestamp }
      })
      .populate('userId', 'name email role department')
      .sort({ timestamp: -1 });
      
      // Group previous events by user ID
      previousEvents.forEach(event => {
        if (event.userId && event.userId._id) {
          const userId = event.userId._id.toString();
          if (!userHistoryMap.has(userId)) {
            userHistoryMap.set(userId, []);
          }
          userHistoryMap.get(userId).push({
            _id: event._id,
    ...
    // and include password reset history
    const transformedEvents = events.map(event => {
      const userId = event.userId?._id?.toString();
      const history = userId ? userHistoryMap.get(userId) || [] : [];
