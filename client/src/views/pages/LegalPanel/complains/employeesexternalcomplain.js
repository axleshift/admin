...
export default function EmployeeComplaintForm() {
  ...
  // Load data from localStorage and URL parameters in useEffect
  useEffect(() => {
    // First, extract data from URL if coming from email link
    const params = new URLSearchParams(window.location.search);
    const urlData = {
      employeeId: params.get('employeeId') || '',
      employeeName: params.get('employeeName') || '',
      employeeEmail: params.get('employeeEmail') || '',
      department: params.get('department') || '',
      employeeUsername: params.get('employeeUsername') || ''
    };
    
    // Check if any URL parameters exist (indicating email source)
    const hasUrlParams = Object.values(urlData).some(val => val !== '');
    
    // If coming from email (URL params exist), use that data
    if (hasUrlParams) {
      console.log('Loading data from URL parameters:', urlData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/complains/employeecomplainexternal', formData);
